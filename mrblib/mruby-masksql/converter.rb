module MrubyMasksql
  class Converter
    ...
    def mask_value(masked_value, original_value, mask_index, columns)
      masked_value = masked_value.insert(0, "'") if original_value.start_with?("'", "('")
      masked_value = masked_value.insert(-1, "'") if original_value.end_with?("'", "')")
      masked_value = masked_value.insert(0, '(') if mask_index == 0
      masked_value = masked_value.insert(-1, ')') if mask_index == columns - 1
      masked_value
    end
  end
end
